...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def __init__(self):
        ...
        LAUNCH_FILE = '/home/yel-wishahy/enph353_gym-gazebo/gym_gazebo/envs/ros_ws/src/enph353_lab06/launch/lab06_world.launch'
    ...
    def process_image(self, data):
        ...
        segment_size = image_gray.shape[1]/len(state)
        x_avg = -1

        # locate path pixels based on colour
        _, X = np.where(np.all(image_gray == gray_clr, axis=2))
        if X.size != 0:
            x_avg = int(np.average(X))
            segment_index = int(x_avg/segment_size)
            if(segment_index >= 0 and segment_index < len(state)):
                state[segment_index] = 1
        
        if(np.max(np.array(state)) < 1):
            self.timeout+=1
